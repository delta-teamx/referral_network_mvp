...
import {
  ...
  Save, Sparkles, Star, Target, Video, X,
} from 'lucide-react';
        ...
        <motion.div variants={fadeInUp} initial="hidden" animate="visible" className="mx-auto max-w-3xl">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white px-6 shadow-sm">
            ...
            {profile.servicesOffered.length > 0 && (
              <Section icon={Star} title="Services offered">
                <div className="flex flex-wrap gap-2">
                  {profile.servicesOffered.map((s) => <Tag key={s}>{s}</Tag>)}
                </div>
              </Section>
            )}
              ...
              {profile.videoUrl ? (
                <div>
                  ...
                  {profile.videoTranscript && (
                    <p className="mt-2 rounded-md bg-gray-50 px-3 py-2 text-xs text-gray-600">{profile.videoTranscript.slice(0, 300)}...</p>
                  )}
                </div>
              ) : showVideoRecorder ? (
                <div>
                  ...
                  />
                  <button onClick={() => setShowVideoRecorder(false)} className="mt-2 text-sm text-gray-500 hover:text-gray-700">Cancel</button>
                </div>
              ) : (
                <button
                  onClick={() => setShowVideoRecorder(true)}
                  className="inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary-light px-4 py-2 text-sm font-medium text-primary hover:bg-primary hover:text-white"
                >
            ...
            {(profile.icpIndustries.length > 0 || profile.icpRoles.length > 0) && (
              <Section icon={Target} title="Who I want to meet">
                {profile.icpIndustries.length > 0 && (
                  <div className="mb-2">
                    <p className="mb-1 text-xs font-medium text-gray-500">Industries</p>
                    <div className="flex flex-wrap gap-2">{profile.icpIndustries.map((s) => <Tag key={s}>{s}</Tag>)}</div>
                  </div>
                )}
                {profile.icpRoles.length > 0 && (
                  <div className="mb-2">
                    <p className="mb-1 text-xs font-medium text-gray-500">Roles</p>
                    <div className="flex flex-wrap gap-2">{profile.icpRoles.map((s) => <Tag key={s}>{s}</Tag>)}</div>
                  </div>
                )}
            ...
            {profile.canReferIndustries.length > 0 && (
              <Section icon={Handshake} title="Who I can refer to">
                <div className="flex flex-wrap gap-2">{profile.canReferIndustries.map((s) => <Tag key={s}>{s}</Tag>)}</div>
              </Section>
            )}

            {/* Barter */}
            {profile.openToBarter && (
              <Section icon={Sparkles} title="Open to bartering">
                {profile.barterOfferings.length > 0 && (
                  <div className="mb-2">
                    <p className="mb-1 text-xs font-medium text-gray-500">I can offer</p>
                    <div className="flex flex-wrap gap-2">{profile.barterOfferings.map((s) => <Tag key={s}>{s}</Tag>)}</div>
                  </div>
                )}
                {profile.barterWants.length > 0 && (
                  <div className="mb-2">
                    <p className="mb-1 text-xs font-medium text-gray-500">I would accept</p>
                    <div className="flex flex-wrap gap-2">{profile.barterWants.map((s) => <Tag key={s}>{s}</Tag>)}</div>
                  </div>
                )}
            ...
            {profile.keywords.length > 0 && (
              <Section icon={Globe} title="Keywords for AI matching">
                <div className="flex flex-wrap gap-2">{profile.keywords.map((s) => <Tag key={s}>{s}</Tag>)}</div>
              </Section>
            )}
          </div>
        </motion.div>
      ...
      <div className="mx-auto max-w-3xl">
        <div className="mb-4 flex items-center justify-between">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full border border-gray-200 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50"
          >
            <X size={14} /> Cancel
          </button>
        </div>
        ...
        <form onSubmit={onSubmit} className="space-y-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <section>
            ...
            <label className="mb-4 flex cursor-pointer items-center gap-3 rounded-lg border border-gray-200 px-4 py-3 text-sm hover:border-primary">
              ...
              <span className="font-medium text-gray-900">Open to bartering services</span>
            </label>
            <div className="space-y-4">

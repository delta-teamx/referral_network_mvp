...
/**
 * "Continue with Google" button. Kicks off the OAuth flow by navigating to
 * the backend start endpoint, which either 302s to Google or to our
 * `/oauth/demo` landing depending on whether credentials are configured.
 */
export function GoogleButton({ label }: { label?: string }) {
  const apiBase =
    (typeof process !== 'undefined' && process.env.NEXT_PUBLIC_API_URL) ||
    (typeof window !== 'undefined' ? window.location.origin : '');
  // In demo mode (no NEXT_PUBLIC_API_URL) send the user straight to
  // /oauth/demo so the button works without any backend whatsoever.
  const href =
    !process.env.NEXT_PUBLIC_API_URL || process.env.NEXT_PUBLIC_DEMO_MODE === 'force'
      ? '/oauth/demo'
      : `${apiBase}/api/v1/auth/oauth/google`;

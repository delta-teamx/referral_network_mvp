...
export async function getUserActivity(userId: string) {
  ...
  const [
    ...
    prisma.businessConnection.count({
      where: { OR: [{ initiatorId: userId }, { targetId: userId }], status: 'accepted' },
    }),
    prisma.introduction.count({ where: { senderId: userId } }),
    ...
    prisma.groupEventRsvp.count({ where: { userId } }).catch(() => 0),
  ...
  return {
    ...
    bookingsHosted: bookingsHost,
    bookingsAsGuest: bookingsGuest,
    bookingsConfirmed,
    eventsRsvped,
    pipeline: Object.fromEntries(pipelineByStage.map((r) => [r.stage, r._count._all])),
    pipelineTotal: pipelineByStage.reduce((sum, r) => sum + r._count._all, 0),
